...
export const requestOTP = async (payload) => {
   const { data } = await apiClient.post(`/publicrides/customer/v2/login?isdev=${true}`, payload);
  // const data = {
  //   success: true,
  //   message: 'OTP Sent',
  // };
...
export const verifyOTP = async (payload) => {
  const { data } = await apiClient.post(`publicrides/customer/v2/verifyotp?isdev=${true}`, payload);
...
export const markBillAsPaid = async (tripId, billIndex) => {
  const { data } = await apiClient.post('/publicrides/customer/v2/markBillAsPaid', { tripId, billIndex });
  return data;
};
...
export const uploadPaymentReceipt = async (tripId, billIndex, imageAsset) => {
  ...
  const { data } = await apiClient.post('/publicrides/customer/v2/uploadPaymentReceipt', formData, {
