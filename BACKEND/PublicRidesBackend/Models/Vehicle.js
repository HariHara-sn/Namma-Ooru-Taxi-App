...
class Vehicles {
  ...
  static addVehicleToPublicRide = async (vehicle) => {
    ...
    if (result.acknowledged) return result;
      ...
      throw new DatabaseInsertFailed(
        "Failed to add vehicle to databse -- Insert Failed",
      );
  };
  ...
  static getPassangerVehicleByVehicleNumber = async (
    passangerId,
    vehicleNumber,
  ) => {
  ...
  static updateVehicleForVMFleets = async (vehicle) => {
    ...
    if (result.acknowledged && result.modifiedCount > 0) return result;
      ...
      throw new DatabaseUpdateFailed(
        "Failed to update vehicle in databse -- Update Failed",
      );
  };
  ...
  static reactivateVehicle = async (vehicle) => {
    ...
    if (result.acknowledged && result.modifiedCount > 0) {
      ...
      throw new DatabaseUpdateFailed(
        "Failed to reactivate vehicle in database -- Update Failed",
      );
    }
  };
  ...
  static deleteVehicleForVMFleets = async (vehicle) => {
    ...
    if (result.acknowledged && result.modifiedCount > 0) return result;
      ...
      throw new DatabaseDeleteFailed(
        "Failed to delete vehicle from databse -- Delete Failed",
      );
  };
  ...
  static addPassangerVehicle = async (vehicleDoc) => {
    ...
    if (result.acknowledged) return result;
      ...
      throw new DatabaseInsertFailed(
        "Failed to add passenger vehicle -- Insert Failed",
      );
  };
  ...
  static deletePassangerVehicle = async (vehicleId) => {
    ...
    return Mongo.deleteOne(COLLECTION_NAME, {
        _id: new ObjectId(vehicleId),
    });
};
}
