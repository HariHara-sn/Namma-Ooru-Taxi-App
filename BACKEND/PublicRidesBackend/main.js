...
const {
  getRecentLocationsSchema,
} = require("./graphql/schemas/LocationGetSchema");
...
function validateRazorpayEnv() {
  const disable =
    (process.env.RAZORPAY_DISABLE_STRICT || "").toLowerCase() === "true";
  const keyId = process.env.RAZORPAY_KEY_ID;
  const keySecret = process.env.RAZORPAY_KEY_SECRET;
  if (!keyId || !keySecret) {
    const msg =
      "[Startup] Missing Razorpay keys (RAZORPAY_KEY_ID / RAZORPAY_KEY_SECRET).";
    if (disable || process.env.NODE_ENV === "test") {
      console.warn(
        msg + " Stub mode allowed due to RAZORPAY_DISABLE_STRICT or test env.",
      );
      return false;
    }
    console.error(
      msg + " Set RAZORPAY_DISABLE_STRICT=true to bypass. Exiting.",
    );
    process.exit(1);
  }
  return true;
